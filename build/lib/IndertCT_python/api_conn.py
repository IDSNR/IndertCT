...
class Client:
    ...
    def historical_data(self, **kwargs):
        ...
        if not "custom data" in kwargs:
             ...
             return self.POSTrequests("historicalCr", {"times": times, "limit": limit, "crs": cryptos, "password": password})

        else:

            custom_data = makevariable(kwargs, "custom data", dict)

            return self.POSTrequests("historicalCr", {"times": times, "limit": limit, "crs": cryptos, "custom data": custom_data, "password": password})
